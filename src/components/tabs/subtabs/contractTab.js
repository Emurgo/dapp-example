    ...
    <>
      <div className="grid justify-items-center py-5 px-5">
        <div className="block p-6 min-w-full rounded-lg border shadow-md bg-gray-800 border-gray-700">
          ...
          <div className="mb-6">
            <label htmlFor="hex" className="block mb-2 text-sm font-medium text-gray-300">
              Contract Hex
            </label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <div className="flex-grow">
                <textarea
                  className="w-full h-full flex-row rounded bg-gray-900 text-white px-2 resize-none"
                  disabled
                  readOnly
                  value={contractSaveText}
                ></textarea>
              </div>
            </div>
          </div>
...
const SendToContract = ({contractInfo}) => {
  ...
  const sendToContract = async () => {
    ...
    const wasmOutput = wasm.TransactionOutput.new(wasmContractAddress, wasmValue)
    ...
    api?.signTx(unsignedTransactionHex).then((witnessSetHex) => {
      const wasmWitnessSet = wasm.TransactionWitnessSet.from_bytes(hexToBytes(witnessSetHex))
      const wasmTx = wasm.Transaction.from_bytes(hexToBytes(unsignedTransactionHex))
      const wasmSignedTransaction = wasm.Transaction.new(wasmTx.body(), wasmWitnessSet, wasmTx.auxiliary_data())
      ...
      <div className="block p-6 min-w-full rounded-lg border shadow-md bg-gray-800 border-gray-700">
        ...
        <div className="flex items-start mb-6">
          <div className="flex items-center h-5">
            <input
              id="inline-datum-check"
              type="checkbox"
              defaultChecked={inlineDatum}
              value={inlineDatum}
              onChange={() => setInlineDatum(!inlineDatum)}
              className="w-4 h-4 bg-gray-50 rounded border border-gray-300 focus:ring-3 focus:ring-blue-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-blue-600 dark:ring-offset-gray-800"
            />
          </div>
          <label htmlFor="inline-datum-check" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">
            Inline Datum
          </label>
          <div className="flex items-center h-5 pl-5">
            <input
              id="inline-script-check"
              type="checkbox"
              defaultChecked={inlineScript}
              value={inlineScript}
              onChange={() => setInlineScript(!inlineScript)}
              className="w-4 h-4 bg-gray-50 rounded border border-gray-300 focus:ring-3 focus:ring-blue-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-blue-600 dark:ring-offset-gray-800"
            />
          </div>
          <label htmlFor="inline-script-check" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">
            Inline Script
          </label>
        </div>
        ...
        {sendTxInfo.value.assets.map((v, idx) => {
          return (
            <div className="bg-gray-900 grid gap-6 md:grid-cols-2 px-5 py-5">
              <textarea
                className="flex-row w-full rounded bg-gray-900 text-white px-2 resize-none"
                disabled
                readOnly
        ...
        <div className="grid justify-items-center pb-2 py-3">
          <button
            className="block text-white font-medium rounded-lg text-sm px-5 py-2.5 text-center bg-gray-600 hover:bg-gray-700 focus:ring-gray-800"
            type="button"
                ...
                <div className="relative w-auto my-6 mx-auto max-w-3xl">
                  ...
                  <div className="bg-gray-900 border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                    ...
                    <div className="bg-gray-900 flex items-start justify-between p-5 rounded-t">
                      <h3 className="text-3xl font-semibold text-white">Asset</h3>
                      <button
                    ...
                    <div className="bg-gray-900 grid gap-6 md:grid-cols-2 px-5 py-5 ">
                      <div>
                        <label htmlFor="policyId" className="block mb-2 text-sm font-medium text-gray-300">
                          Policy Id (in hex)
                        </label>
                        <input
                          type="text"
                          ...
                          value={currentAssetInfo.policyId}
                          onChange={(event) => setCurrentAssetInfo({...currentAssetInfo, policyId: event.target.value})}
                        />
                      </div>
                      <div>
...
const RedeemFromContract = ({contractInfo}) => {
  ...
  const redeemFromContract = async () => {
    ...
    const wasmRedeemer = wasm.Redeemer.new(
      ...
      wasmRedeemData,
      wasm.ExUnits.new(wasm.BigNum.from_str('942996'), wasm.BigNum.from_str('346100241')),
    )
    ...
    const wasmTxInput = wasm.TransactionInput.new(
      wasm.TransactionHash.from_bytes(hexToBytes(transactionHash)),
      outputId,
    )
    ...
    const wasmContractAddress = wasm.Address.from_bech32(
      'addr_test1wrh5pj6nlmdrmtv6uv69edjh5x3gx7px7zchxag47s23gtgu02rzy',
    )
    const wasmOutput = wasm.TransactionOutput.new(wasmContractAddress, wasm.Value.new(wasm.BigNum.from_str('2000000')))
    ...
    const cost_model_vals = [
      ...
      205665, 812, 1, 1, 41182, 32, 212342, 32, 31220, 32, 32696, 32, 43357, 32, 32247, 32, 38314, 32, 20000000000,
    ...
    api
      ...
      .then((witnessSetHex) => {
        ...
        const wasmWitnessSetCopy = wasm.TransactionWitnessSet.from_bytes(hexToBytes(witnessSetHex))
  ...
  return (
    <div className="block p-6 min-w-full rounded-lg border shadow-md bg-gray-800 border-gray-700">
      <div className="bg-gray-800 grid gap-6 md:grid-cols-2 px-5 py-5 ">
        <div>
          <label htmlFor="transactionId" className="block mb-2 text-sm font-medium text-gray-300">
            Transaction Id
          </label>
          <input
            type="text"
            id="transactionId"
            className="appearance-none border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500"
            value={scriptInputInfo.transactionHash}
            onChange={(event) => setScriptInputInfo({...scriptInputInfo, transactionHash: event.target.value})}
          />
        </div>
        <div>
          <label htmlFor="outputId" className="block mb-2 text-sm font-medium text-gray-300">
            output Id
          </label>
          <input
            type="text"
      ...
      <div className="flex justify-between px-5 pb-5">
        <div className="flex items-start mb-6">
          <div className="flex items-center h-5">
            <input
              id="inlined-datum-check"
              type="checkbox"
              defaultChecked={inlinedDatum}
              value={inlinedDatum}
              onChange={() => setInlinedDatum(!inlinedDatum)}
              className="w-4 h-4 bg-gray-50 rounded border border-gray-300 focus:ring-3 focus:ring-blue-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-blue-600 dark:ring-offset-gray-800"
            />
          </div>
          <label htmlFor="inlined-datum-check" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">
            Inlined Datum
          </label>
          <div className="flex items-center h-5 pl-5">
            <input
              id="inlined-script-check"
              type="checkbox"
              defaultChecked={inlinedScript}
              value={inlinedScript}
              onChange={() => setInlinedScript(!inlinedScript)}
              className="w-4 h-4 bg-gray-50 rounded border border-gray-300 focus:ring-3 focus:ring-blue-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-blue-600 dark:ring-offset-gray-800"
            />
          </div>
          <label htmlFor="inlined-script-check" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">
            Inlined Script
          </label>
        </div>
        <button
